...
class AWT_Plugin_Tabs
{
    ...
    public function save_product($post_id, $post, $update)
    {
        ...
        update_post_meta($post_id, 'awt_tabs', $tabs);
    }
}
...
class AWT_Plugin_Tab
{
    ...
    {
        $tabs = array_filter((array)get_option('awt_tabs'));
        $data = (array)get_post_meta($post->ID, 'awt_tabs', true);
?>
        ...
        $i = 0;
        foreach ($tabs as $awt_tab) {
        ?><div class="tab-content <?php echo $i == 0 ? 'tab-content-active' : ''; ?>" id="tab-<?php echo $awt_tab['name']; ?>">
                <?php
                $settings = array(
